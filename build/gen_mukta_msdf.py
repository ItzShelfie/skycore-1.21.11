...
jobs = [
    ("muktaregular", '" ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,:-_/·°%[]()"'),
    ("muktamedium", '" ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,:-_/·°%[]()"'),
]

for out, spec in jobs:
    charset = fonts / f"_charset_{out}.txt"
    charset.write_text(spec + "\n", encoding="utf-8")
    cmd = [
        str(exe), "-font", str(fonts / f"{out}.ttf"),
        "-type", "msdf", "-format", "png",
        "-imageout", str(fonts / f"{out}.png"),
        "-json", str(fonts / f"{out}.json"),
        "-size", "64", "-pxrange", "10", "-yorigin", "bottom",
        "-charset", str(charset),
    ]
    print("GEN", out)
