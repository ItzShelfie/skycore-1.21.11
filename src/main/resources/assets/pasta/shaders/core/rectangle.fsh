...
vec4 getGradientColor(vec2 uv) {
    vec4 topColor = lerpColor(color1, color2, uv.x);
    vec4 bottomColor = lerpColor(color3, color4, uv.x);
    vec4 finalColor = lerpColor(topColor, bottomColor, uv.y);
    return finalColor;
}

vec4 getStrokeColor(vec2 uv) {
    vec4 topColor = lerpColor(stroke1, stroke2, uv.x);
    vec4 bottomColor = lerpColor(stroke3, stroke4, uv.x);
    vec4 finalColor = lerpColor(topColor, bottomColor, uv.y);
    return finalColor;
}
